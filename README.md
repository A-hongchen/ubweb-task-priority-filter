...
本脚本是独立创建的新脚本，不依赖 `UBWEB 豆瓣信息自动填充 - 增强版`。
...
脚本不会主动修改任务的其他字段；已存在 `【优先级：1】` 到 `【优先级：6】` 格式前缀的备注会直接跳过。
...
2. 提升脚本头里的 `@version`，例如 `1.0.0` -> `1.0.1`。
...
如果备注原本已有内容，优先级会写在原备注内容前面；如果备注已经以 `【优先级：X】` 开头，则该任务会跳过。
